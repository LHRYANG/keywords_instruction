...
from V2.prompt_builder import build_add_remove_prompt
import torch 
...
if __name__ == "__main__":
    ...
    samplers = create_attribute_samplers(attribute_folder, avoid_name="") 
        ...
        for complexity in [1, 2, 3, 0]:
            ...
            for i in range(200):
                ...
                if  prob < 0.8:
                    ...
                    length = 3
                sampled_object = object_sampler_insider.sample(length=length, k=args.batch_size, alpha=0.25)
                #print(sampled_object)


                reference_text = sample_reference_text(samplers, k=5, kk=10)
                #print("reference_text:", reference_text)
                prompts = [build_add_remove_prompt(obj, reference_text, max_attributes=complexity) for obj in sampled_object]
                #print(prompts[0])
                batch_results = batch_generate(model, tokenizer, sampled_object, prompts, outtype="json")
