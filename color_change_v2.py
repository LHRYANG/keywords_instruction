...
from V2.prompt_builder import build_motion_prompt, build_color_change_prompt
import torch 
...
if __name__ == "__main__":
    ...
    color_file1 = "inside_keywords/edit_type_color_alter_change_by_length.json"
    color_file2 = "outside_keywords/keyword_color.json"

    color_sampler_insider = KeywordSampler(color_file1)
    color_sampler_outsider = TypedKeywordSampler(color_file2)
    ...
    for kkk in range(0, args.rounds):
        for complexity in [1, 2, 3, 0]:
            ...
            for i in range(200):
                ...
                for line in sampled_color_2:
                    if ':' in line:
                        _, keywords_str = line.split(':', 1)
                        keywords = [kw.strip() for kw in keywords_str.split(',')]
                        merged.update(keywords)
                sampled_color = list(merged)

                reference_text = sample_reference_text(samplers, k=5, kk=10)
                #print("reference_text:", reference_text)
                prompts = [build_color_change_prompt(obj, reference_text, sampled_color, max_attributes=complexity) for obj in sampled_object]
                #print(prompts[0])
